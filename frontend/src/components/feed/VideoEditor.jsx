// ============================================================
// SPYCE - VideoEditor v5
// ADDED: Save Mix button in audio panel
// ADDED: Music actually plays alongside video in preview
...
function DraggableItem({ item, containerRef, onMove, onRemove, children }) {
  ...
  const onStart = useCallback((e) => {
    ...
    dragging.current=true; startPos.current=getPos(e); startItem.current={x:item.x,y:item.y};
  },[item.x,item.y]);
  const onMove_ = useCallback((e) => {
    if(!dragging.current) return; e.preventDefault();
    const pos=getPos(e), rect=containerRef.current?.getBoundingClientRect();
    if(!rect) return;
...
export default function VideoEditor({ videoSrc, selectedTrack, onEditsChange, edits }) {
  ...
  const musicAudioRef = useRef(null); // Music audio for preview mixing
  ...
  const [mixSaved, setMixSaved] = useState(edits?.mixSaved||false);
  ...
  const [trimVideoSrc] = useState(videoSrc);

  // When audio tab opens, start playing music preview alongside video
  useEffect(() => {
    if (activeTab === 'audio' && selectedTrack?.previewUrl) {
      if (!musicAudioRef.current) {
        const a = new Audio(selectedTrack.previewUrl);
        a.loop = true;
        a.volume = musicMuted ? 0 : musicVolume / 100;
      ...
      // Stop music when leaving audio tab
      if (musicAudioRef.current) {
    ...
    return () => {
      if (musicAudioRef.current && activeTab !== 'audio') {
        musicAudioRef.current.pause();
        musicAudioRef.current = null;
        setMusicPlaying(false);
      }
  ...
  // Apply volume changes to live audio
  useEffect(() => {
  ...
  // Trim duration loading
  useEffect(() => {
    if(activeTab!=='trim') return;
    const v=trimVideoRef.current; if(!v) return;
    setLoadingDuration(true);
    const load=()=>{
      if(v.duration&&isFinite(v.duration)&&v.duration>0){
        setDuration(v.duration); setTrimEnd(prev=>prev||v.duration); setLoadingDuration(false);
      }
    };
    if(v.readyState>=1&&v.duration>0){load();return;}
    v.load();
    v.addEventListener('loadedmetadata',load); v.addEventListener('durationchange',load); v.addEventListener('canplay',load);
    return()=>{ v.removeEventListener('loadedmetadata',load); v.removeEventListener('durationchange',load); v.removeEventListener('canplay',load); };
  },[activeTab]);

  useEffect(()=>{
    const v=trimVideoRef.current; if(!v||!trimPlaying) return;
    const onTime=()=>{ setCurrentTime(v.currentTime); if(trimEnd&&v.currentTime>=trimEnd) v.currentTime=trimStart; };
  ...
  const notify = useCallback((extra={})=>{
    onEditsChange?.({texts,stickers,filter,videoVolume,musicVolume,musicMuted,trimStart,trimEnd:trimEnd||duration||0,mixSaved,...extra});
  },[texts,stickers,filter,videoVolume,musicVolume,musicMuted,trimStart,trimEnd,duration,mixSaved]);

  // Save mix
  const handleSaveMix = () => {
    setMixSaved(true);
    notify({mixSaved:true, videoVolume, musicVolume, musicMuted});
    toast.success(`✅ Mix saved! 🎬${videoVolume}% + 🎵${musicMuted?'Off':musicVolume+'%'}`);
  };

  const addText=()=>{
    if(!textInput.trim()) return;
    const t=[...texts,{id:Date.now(),text:textInput,color:textColor,size:textSize,x:50,y:50}];
    setTexts(t); setTextInput(''); notify({texts:t}); setActiveTab(null); toast.success('Text added! Drag to move.');
  };
  ...
  const addSticker=(emoji)=>{
    ...
    setStickers(s); notify({stickers:s}); toast.success(`${emoji} added! Drag to move.`);
  };
  ...
  const applyTrim=()=>{
    ...
    setTrimApplied(true); notify({trimStart,trimEnd:trimEnd||duration});
    toast.success(`✂️ Trimmed to ${d.toFixed(1)}s`); setActiveTab(null);
  };
  ...
  return (
    <div style={{width:'100%'}}>
      ...
      <div ref={previewRef} style={{position:'relative',width:'100%',height:'200px',borderRadius:'16px',overflow:'hidden',background:'#000'}}>
        ...
        {/* Music playing indicator in preview */}
        {musicPlaying && selectedTrack && (
          <div style={{position:'absolute',bottom:'8px',left:'8px',background:'rgba(0,0,0,0.6)',backdropFilter:'blur(4px)',borderRadius:'20px',padding:'4px 10px',fontSize:'0.7rem',color:'white',display:'flex',alignItems:'center',gap:'6px',pointerEvents:'none'}}>
            <div style={{display:'flex',gap:'2px',alignItems:'flex-end',height:'12px'}}>
              {[1,2,3].map(i=><div key={i} style={{width:'2px',background:'#ff3c5f',borderRadius:'1px',animation:`eq${i} ${0.3+i*0.1}s ease-in-out infinite alternate`,minHeight:'3px'}}/>)}
            </div>
            {selectedTrack.title}
          </div>
        )}
      ...
      {/* Hidden trim video */}
      <video ref={trimVideoRef} src={trimVideoSrc} style={{display:'none'}} preload="metadata"/>
      ...
      {activeTab==='trim'&&(
        <div style={{marginTop:'10px',background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:'14px',padding:'14px'}}>
          <div style={{position:'relative',width:'100%',height:'120px',borderRadius:'10px',overflow:'hidden',background:'#000',marginBottom:'12px'}}>
            <video ref={trimVideoRef} src={videoSrc} style={{width:'100%',height:'100%',objectFit:'cover'}} playsInline preload="auto" muted/>
            {loadingDuration&&(
            ...
            {!loadingDuration&&duration&&(
              <div style={{position:'absolute',bottom:'6px',right:'8px',background:'rgba(0,0,0,0.6)',borderRadius:'6px',padding:'2px 8px',fontSize:'0.7rem',color:'white'}}>{fmt(currentTime)} / {fmt(duration)}</div>
            )}
          </div>
          {!loadingDuration&&duration?(
            <>
              <p style={{fontSize:'0.75rem',color:'var(--text-secondary)',marginBottom:'10px',textAlign:'center'}}>Drag sliders · Total: {fmt(duration)}</p>
              <div style={{position:'relative',height:'36px',background:'var(--bg-elevated)',borderRadius:'8px',overflow:'hidden',marginBottom:'12px'}}>
                <div style={{position:'absolute',top:0,bottom:0,left:`${(trimStart/effectiveDuration)*100}%`,width:`${((effectiveTrimEnd-trimStart)/effectiveDuration)*100}%`,background:'rgba(255,60,95,0.35)',border:'2px solid var(--brand-red)',borderRadius:'4px'}}/>
                {duration>0&&<div style={{position:'absolute',top:0,bottom:0,left:`${(currentTime/duration)*100}%`,width:'2px',background:'white'}}/>}
              ...
              {trimApplied&&<div style={{marginTop:'10px',padding:'8px 12px',background:'rgba(74,222,128,0.1)',border:'1px solid rgba(74,222,128,0.3)',borderRadius:'10px',fontSize:'0.78rem',color:'#4ade80',textAlign:'center'}}>✓ Trimmed to {fmt(effectiveTrimEnd-trimStart)}</div>}
            </>
      ...
      {activeTab==='audio'&&(
        <div style={{marginTop:'10px',background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:'14px',padding:'16px'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'4px'}}>
            <p style={{fontSize:'0.85rem',fontWeight:700,fontFamily:'var(--font-display)'}}>🎛️ Audio Mixer</p>
            {mixSaved&&<span style={{fontSize:'0.7rem',color:'#4ade80',fontWeight:600}}>✓ Saved</span>}
          </div>
          <p style={{fontSize:'0.75rem',color:'var(--text-muted)',marginBottom:'16px'}}>
            {selectedTrack&&musicPlaying ? '🎵 Music preview playing — adjust levels' : 'Adjust original sound volume'}
          </p>
          ...
          <div style={{marginBottom:'20px'}}>
            ...
            <input type="range" min="0" max="100" value={videoVolume}
              onChange={e=>{const v=parseInt(e.target.value);setVideoVolume(v);if(mainVideoRef.current)mainVideoRef.current.volume=v/100;setMixSaved(false);}}
              style={{width:'100%',accentColor:'var(--brand-red)',height:'6px',cursor:'pointer',marginBottom:'8px'}}/>
            <div style={{display:'flex',gap:'6px'}}>
              {[0,50,80,100].map(v=>(
                <button key={v} onClick={()=>{setVideoVolume(v);if(mainVideoRef.current)mainVideoRef.current.volume=v/100;setMixSaved(false);}} style={{flex:1,padding:'5px',borderRadius:'8px',fontSize:'0.7rem',cursor:'pointer',background:videoVolume===v?'var(--brand-gradient)':'var(--bg-elevated)',border:`1px solid ${videoVolume===v?'transparent':'var(--border)'}`,color:videoVolume===v?'white':'var(--text-muted)'}}>{v===0?'Off':`${v}%`}</button>
              ))}
            </div>
          </div>
          ...
          {selectedTrack?(
            <div style={{marginBottom:'20px'}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:'8px'}}>
                <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                  {selectedTrack.artwork?<img src={selectedTrack.artwork} alt="" style={{width:'28px',height:'28px',borderRadius:'6px'}}/>:<span style={{fontSize:'18px'}}>🎵</span>}
                  <div>
                    <p style={{fontSize:'0.82rem',fontWeight:600,lineHeight:1.2}}>{selectedTrack.title}</p>
                    <p style={{fontSize:'0.7rem',color:'var(--text-muted)'}}>{selectedTrack.artist}</p>
                  </div>
                </div>
              ...
              <input type="range" min="0" max="100" value={musicMuted?0:musicVolume} disabled={musicMuted}
                onChange={e=>{const v=parseInt(e.target.value);setMusicVolume(v);setMusicMuted(false);if(musicAudioRef.current)musicAudioRef.current.volume=v/100;setMixSaved(false);}}
                style={{width:'100%',accentColor:'var(--pi-gold)',height:'6px',cursor:'pointer',opacity:musicMuted?0.4:1,marginBottom:'8px'}}/>
              <div style={{display:'flex',gap:'6px'}}>
                {[0,50,70,100].map(v=>(
                  <button key={v} onClick={()=>{
                    if(v===0){setMusicMuted(true);if(musicAudioRef.current)musicAudioRef.current.volume=0;}
                    else{setMusicVolume(v);setMusicMuted(false);if(musicAudioRef.current)musicAudioRef.current.volume=v/100;}
                    setMixSaved(false);
            ...
            <div style={{padding:'14px',textAlign:'center',background:'var(--bg-elevated)',borderRadius:'12px',marginBottom:'16px'}}>
          ...
          <button
            ...
            style={{
              width:'100%',
              padding:'14px',
              background: mixSaved ? 'rgba(74,222,128,0.15)' : 'var(--brand-gradient)',
              border: mixSaved ? '1.5px solid rgba(74,222,128,0.4)' : 'none',
              borderRadius:'12px',
              color: mixSaved ? '#4ade80' : 'white',
          ...
          >
            {mixSaved ? (
              <>✅ Mix Saved · 🎬{videoVolume}% + 🎵{musicMuted?'Off':musicVolume+'%'}</>
            ) : (
              <>💾 Save Mix Settings</>
            )}
          </button>
          {!mixSaved&&<p style={{textAlign:'center',fontSize:'0.7rem',color:'var(--text-muted)',marginTop:'6px'}}>Save your audio levels before posting</p>}
        </div>
      )}
