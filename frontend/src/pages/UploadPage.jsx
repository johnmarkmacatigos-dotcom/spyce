// ============================================================
// SPYCE - Upload Page v5 — COMPLETE FILE
// - No file size limit (user can select any video)
// - Trim panel to cut video before upload
// - Direct Cloudinary upload (no Render timeout)
// - Music search with preview
// - Audio mixer (original sound + music levels)
// - Text, Stickers, Filters editor
  ...
  return (
    <div style={{
      ...
      <button onClick={() => onPlay(track)} style={{
        ...
        <img src={track.artworkUrl100 || track.artworkUrl60} alt="" style={{
      ...
      <div style={{ flex: 1, minWidth: 0, padding: '12px 6px 12px 0' }}>
        ...
        <p style={{ fontSize: '0.71rem', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {track.artistName}{track.trackTimeMillis && <span style={{ opacity: 0.6 }}> · {fmt(track.trackTimeMillis)}</span>}
        </p>
        {isPlaying && <p style={{ fontSize: '0.62rem', color: 'var(--brand-red)', marginTop: '3px', fontWeight: 700 }}>♪ PREVIEW PLAYING</p>}
      </div>
...
export default function UploadPage() {
  ...
  const [edits, setEdits] = useState({ texts: [], stickers: [], filter: null, trimStart: 0, trimEnd: 0, videoVolume: 80, musicVolume: 70 });
  ...
  // File selection — NO SIZE LIMIT (trim handles large files)
  const handleFile = (e) => {
    const f = e.target.files[0];
    if (!f) return;
    if (!f.type.startsWith('video/')) { toast.error('Select a video file'); return; }
    // No size limit — user can trim large videos
    cancelAnimationFrame(animRef.current);
    setFile(f);
    setPreview(URL.createObjectURL(f));
    if (f.size > 100 * 1024 * 1024) {
      toast('Large file detected. Use ✂️ Trim to reduce size before uploading.', {
  ...
  const playPreview = (track) => {
    ...
    a.onended = () => { clearInterval(previewTimer.current); setPreviewProgress(0); setPlayingId(null); audioRef.current = null; };
    audioRef.current = a;
  ...
  const handleUpload = async () => {
    ...
    // Warn if over 100MB and not trimmed
    if (file.size > 100 * 1024 * 1024 && !edits.trimStart && !edits.trimEnd) {
      toast('Tip: Use ✂️ Trim first to reduce file size for faster upload!', { icon: '⚠️', duration: 4000 });
    }
  ...
  const QUICK = ['trending', 'OPM', 'hip hop', 'pop 2025', 'R&B', 'EDM', 'love song'];
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg-primary)', paddingBottom: '90px' }}>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {!file ? (
            <>
              ...
              <div style={{
                ...
                <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                  Any size · MP4 · MOV · Use Trim for large files
                </p>
              </div>
            </>
          ) : (
            <>
              ...
              {file && (
                <div style={{
                  position: 'absolute', top: '10px', left: '10px',
                  background: file.size > 100 * 1024 * 1024
                    ? 'rgba(240,165,0,0.85)' : 'rgba(0,0,0,0.55)',
                  borderRadius: '8px', padding: '3px 10px',
                  fontSize: '0.7rem', fontWeight: 700, color: 'white',
                  backdropFilter: 'blur(4px)',
                }}>
                  {file.size > 100 * 1024 * 1024 ? '⚠️ ' : ''}
                  {(file.size / (1024 * 1024)).toFixed(1)}MB
                  {file.size > 100 * 1024 * 1024 ? ' — use ✂️ Trim!' : ''}
                </div>
              )}
        ...
        {/* Video Editor (trim, audio, text, stickers, filters) */}
        {file && preview && (
          <VideoEditor
            videoSrc={preview}
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                <button onClick={() => setShowMusic(true)} style={{
                  ...
                  padding: '5px 10px', fontSize: '0.72rem', color: 'var(--text-secondary)', cursor: 'pointer',
                }}>Change</button>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', padding: '10px 12px', borderBottom: '1px solid var(--border)' }}>
                  {QUICK.map(tag => (
                    <button key={tag} onClick={() => setMusicQuery(tag)} style={{
                      ...
                      fontSize: '0.75rem', color: 'var(--text-secondary)', whiteSpace: 'nowrap', cursor: 'pointer',
                    }}>{tag}</button>
                  ))}
                </div>
              )}
              ...
              <div style={{ maxHeight: '280px', overflowY: 'auto' }}>
                ...
                {!musicLoading && musicQuery && musicResults.length === 0 && (
                  <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.85rem' }}>No results for "{musicQuery}"</div>
                )}
                {!musicLoading && !musicQuery && (
                  <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.82rem' }}>🎵 Search millions of songs above</div>
                )}
